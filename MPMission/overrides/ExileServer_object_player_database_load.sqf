...
_player setVariable ["ExileScore", (_data select 39), true];
_player setVariable ["ExileKills", (_data select 40), true];
_player setVariable ["ExileDeaths", (_data select 41), true];
...
//_player setVariable ["ExileKillStack", (_data select 47), true];

//******************* START CUSTOM
_player setVariable ["KFB_totalMineWeight", 0, true];
_player setVariable ["KFB_mineAddAction", false, true];
_player setVariable ["KFB_ownedMines", [], true];
//******************* END

diag_log format ["KILL STACK for '%1': %2", _name, (_data select 47)];
	...
	_devs = getArray (configFile >> "CfgSettings" >> "ServerSettings" >> "devs");
	{
		if (_playerUID isEqualTo (_x select 0)) exitWith 
		{
			if (_name isEqualTo (_x select 1)) then
			{
				...
				_player setVariable ["ExileScore", 100000, true];
			};
		};
	}
	forEach _devs;
...
[_player] call ExileServer_MineLimiter_network_setMineOwnership;
